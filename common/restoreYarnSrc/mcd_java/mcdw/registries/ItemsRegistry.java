...
import static mcd_java.mcdw.Mcdw.ID;
    ...
    public static void register() {
        ...
        for (ItemsID itemID : ItemsID.values()) {
            if (!CONFIG.mcdwEnableItemsConfig.ITEMS_ENABLED.get(itemID))
                continue;
            Item item = itemID.makeItem(new Item.Properties());
            registerItem(itemID.toString().toLowerCase(Locale.ROOT), item);
            ItemGroupEvents.modifyEntriesEvent(CreativeModeTabs.INGREDIENTS).register(entries -> entries.accept(itemID.getItem()));
        }
    }

    protected static void registerItem(String id, Item item) {
        Registry.register(BuiltInRegistries.ITEM, ID(id), item);
    }
