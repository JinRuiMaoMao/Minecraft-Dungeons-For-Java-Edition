...
public class LootTablesRegistry {
    ...
    public static void register() {
        LootTableEvents.MODIFY.register((resourceManager, lootManager, id, tableBuilder, source) -> {
            ...
            if (CONFIG.mcdwNewlootConfig.WEAPONS_ENABLED_IN_LOOTTABLES.get(SettingsID.ENABLE_WEAPONS_IN_LOOT_TABLES)) {
                ...
                lootPoolBuilder.when(LootItemRandomChanceCondition.randomChance(CONFIG.mcdwNewlootConfig.findWeaponChance));
                lootPoolBuilder.setBonusRolls(ConstantValue.exactly(CONFIG.mcdwNewlootConfig.bonusRollsWithLuck));
                ...
                if (COMMON_LOOT_TABLES.contains(id.toString())) {
                    ...
                    if (Arrays.stream(lootPoolBuilder.build().entries).noneMatch(lootPoolEntry ->
                            lootPoolEntry.equals(LootItem.lootTableItem(CrossbowsID.CROSSBOW_PRIDE_OF_THE_PIGLINS.getItem())
    ...
    public static void addWeapon(LootPool.Builder lootPoolBuilder, Item weapon, int weight) {
        lootPoolBuilder.add(LootItem.lootTableItem(weapon).setWeight(weight));
    }
    ...
    public static void addItemDrop(LootPool.Builder lootPoolBuilder, Item item, int count, float probability) {
        lootPoolBuilder.setRolls(ConstantValue.exactly(count));
        lootPoolBuilder.when(LootItemRandomChanceCondition.randomChance(probability));
