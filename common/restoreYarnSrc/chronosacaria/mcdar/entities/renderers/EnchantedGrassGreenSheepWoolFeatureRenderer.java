...
public class EnchantedGrassGreenSheepWoolFeatureRenderer extends RenderLayer<EnchantedGrassGreenSheepEntity, SheepModel<EnchantedGrassGreenSheepEntity>> {
    private static final ResourceLocation SKIN = new ResourceLocation("textures/entity/sheep/sheep_fur.png");
    private final SheepFurModel<EnchantedGrassGreenSheepEntity> model;
    public EnchantedGrassGreenSheepWoolFeatureRenderer(RenderLayerParent<EnchantedGrassGreenSheepEntity, SheepModel<EnchantedGrassGreenSheepEntity>> context, EntityModelSet loader) {
        super(context);
    ...
    @Override
    public void render(PoseStack matrixStack, MultiBufferSource vertexConsumerProvider, int light, EnchantedGrassGreenSheepEntity sheepEntity, float limbAngle, float limbDistance, float tickDelta, float animationProgress, float headYaw, float headPitch) {
        float u;
        float t;
        float s;
        ...
        if (sheepEntity.hasCustomName() && "Lilly".equals(sheepEntity.getName().getString())) {
            ...
            float[] hs = Sheep.getColorArray(DyeColor.GREEN);
            s = hs[0];
            t = hs[1];
            u = hs[2];
        }
