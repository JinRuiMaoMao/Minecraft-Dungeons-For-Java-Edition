...
public class LootRegistry {
    ...
    private static void mcdar$artifactsGeneralLootBuilder(List<IArtifactItem> artifactItemList, Identifier id, FabricLootTableBuilder tableBuilder) {
        ...
        lootPoolBuilder.setRolls(ConstantLootNumberProvider.exactly(1))
                .when(RandomChanceLootCondition.randomChance(Mcdar.CONFIG.mcdarArtifactsStatsConfig.ARTIFACT_GENERAL_SPAWN_CHANCE));
    ...
    private static void mcdar$artifactsDungeonLootBuilder(Identifier id, FabricLootTableBuilder tableBuilder) {
        LootPool.Builder lootPoolBuilder = LootPool.lootPool();
        lootPoolBuilder.setRolls(ConstantLootNumberProvider.exactly(1))
                .when(RandomChanceLootCondition.randomChance(Mcdar.CONFIG.mcdarArtifactsStatsConfig.ARTIFACT_DUNGEON_SPAWN_CHANCE));
