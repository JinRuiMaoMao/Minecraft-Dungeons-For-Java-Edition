...
public class CleanlinessHelper {
    ...
    public static void createLoreTTips(ItemStack stack, List<Text> tooltip) {
        ...
        for (int i = 1; i <= 32; i++) {
            Text line = Text.translatable(translationKey + i);
            String key = translationKey + i;
            if (line.getString().equals(key)) {
                break;
            }
            tooltip.add(line.copy().formatted(Formatting.ITALIC));
        }
    }
    ...
    public static TypedActionResult<ItemStack> mcdar$cleanUseWithOptionalStatus(
        ...
        if (!player.isCreative())
            itemStack.damage(1, player, (entity) -> entity.sendEquipmentBreakStatus(hand));
                ...
                if (SummoningHelper.mcdar$summonSummonableEntity(
                    ...
                    if (!itemUsageContextPlayer.isCreative())
                        itemUsageContext.getStack().damage(1, itemUsageContextPlayer,
                                (entity) -> entity.sendEquipmentBreakStatus(itemUsageContext.getHand()));
