...
public class McdarEnchantmentHelper {
    ...
    public static void mcdar$cooldownHelper(PlayerEntity player, Item item, int maxCooldown) {
        int cooldownLevel = mcdar$getCooldownLevel(EnchantsRegistry.COOLDOWN, PlayerEntity);
    ...
    public static int mcdar$getCooldownLevel(Enchantment enchantment, PlayerEntity playerEntity){
        ...
        for (ItemStack itemStack : enchantment.getSlotItems(playerEntity).values())
            totalLevel += net.minecraft.enchantment.EnchantmentHelper.getItemEnchantmentLevel(enchantment, itemStack);
