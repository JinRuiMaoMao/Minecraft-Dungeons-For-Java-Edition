...
public class MCDEnchantments implements ModInitializer {
    ...
    public static final Identifier SYNC_CONFIG_PACKET = Identifier.tryBuild(MOD_ID, "sync_config");
	...
	public void onInitialize() {
        ...
        ResourceManagerHelper.get(PackType.SERVER_DATA)
            ...
            public Identifier getFabricId() {
                return Identifier.tryBuild(MOD_ID, "config");
            }
            @Override
        ...
        ServerLifecycleEvents.SYNC_DATA_PACK_CONTENTS.register((PlayerEntity, joined) -> {
            var buf = PacketByteBufs.create();
            config.writeToClient(buf);
            ServerPlayNetworking.send(PlayerEntity, SYNC_CONFIG_PACKET, buf);
            if (Config.lastError != null) {
                PlayerEntity.sendSystemMessage(Text.literal("[MCDEnchantments]: ")
                        .append(Config.lastError).withStyle(Formatting.RED));
            }
        });
	}
}
