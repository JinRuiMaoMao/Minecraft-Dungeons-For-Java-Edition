...
public class IdentifierGlobbedList {
    ...
    public IdentifierGlobbedList(Map<String, List<String>> nested) {
        for (var kvp : nested.entrySet()) {
            ...
            for (var path : paths) {
                if (path.startsWith("#")) {
                    tags.add(Identifier.tryBuild(namespace, path.substring(1)));
                } else {
                    fullySpecified.add(Identifier.tryBuild(namespace, path));
                }
            }
        }
    }
    ...
    public boolean contains(Identifier id) {
        return containsNamespaceGlob(id) ||
            ...
            namespaceTags.stream().flatMap(ns -> Registries.ENCHANTMENT.getTagNames().filter(tag -> tag.location().getNamespace().equals(ns)))
                .anyMatch(tag -> ModTags.isIn(Registries.ENCHANTMENT.get(id), tag));
    }

    public boolean contains(Enchantment enchantment) {
        return contains(Registries.ENCHANTMENT.getKey(enchantment));
    }
