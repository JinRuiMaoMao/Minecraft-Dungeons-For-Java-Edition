...
import static mcd_java.mcda.enchants.EnchantID.HEAL_ALLIES;
...
public class AOEHelper {
    ...
    public static List<LivingEntity> getAoeTargets(LivingEntity center, LivingEntity attacker, float distance) {
        return center.getWorld().getEntitiesOfClass(LivingEntity.class,
                ...
                (nearbyEntity) -> AbilityHelpe.isAoeTarget(nearbyEntity, attacker, center)
        );
    }

    public static List<LivingEntity> getAttackersOfEntities(LivingEntity affectedEntity, float distance) {
        return affectedEntity.getWorld().getEntitiesOfClass(LivingEntity.class,
    ...
    public static void burnNearbyEnemies(LivingEntity attacker, float damage, float distance) {
        ...
        for (LivingEntity nearbyEntity : nearbyEntities) {
            if (nearbyEntity == null) return;
            nearbyEntity.hurt(nearbyEntity.getWorld().damageSources().onFire(), damage);
        }
    }

    public static void causeExplosion(LivingEntity user, LivingEntity target, float damageAmount, float distance) {
        Explosion explosion = new Explosion(target.getWorld(), target, 5, 5, 5, 3, false, Explosion.BlockInteraction.KEEP);
        for (LivingEntity nearbyEntity : getAoeTargets(target, user, distance)) {
            nearbyEntity.hurt(nearbyEntity.getWorld().damageSources().explosion(explosion), damageAmount);
        }
    }
