...
public class LootTablesRegistry {
    ...
    static final List<IMcdwWeaponID> COMMON_LOOT_POOL = List.of(AxesID.AXE_ANCHOR, AxesID.AXE_AXE, DaggersID.DAGGER_DAGGER, DoubleAxesID.DOUBLE_AXE_DOUBLE, GauntletsID.GAUNTLET_GAUNTLET,
    ...
    static final List<IMcdwWeaponID> UNCOMMON_LOOT_POOL = List.of(BowsID.BOW_BONEBOW, BowsID.BOW_BUBBLE_BOW, BowsID.BOW_HUNTERS_PROMISE, BowsID.BOW_HUNTING_BOW, BowsID.BOW_MASTERS_BOW,
    ...
    static final List<IMcdwWeaponID> RARE_LOOT_POOL = List.of(AxesID.AXE_ENCRUSTED_ANCHOR, AxesID.AXE_FIREBRAND, AxesID.AXE_HIGHLAND, BowsID.BOW_BUBBLE_BURSTER, BowsID.BOW_BURST_GALE_BOW,
    ...
    public static void register() {
        LootTableEvents.MODIFY.register((resourceManager, lootManager, id, tableBuilder, source) -> {
            if (id.equals(EntityType.BEE.getDefaultLootTable()) && source.isBuiltin()) {
            ...
            if (id.equals(EntityType.WITCH.getDefaultLootTable()) && source.isBuiltin()) {
            ...
            if (id.equals(EntityType.WITHER.getDefaultLootTable()) && source.isBuiltin()) {
            ...
            if (CONFIG.mcdwNewlootConfig.WEAPONS_ENABLED_IN_LOOTTABLES.get(SettingsID.ENABLE_WEAPONS_IN_LOOT_TABLES)) {
                ...
                lootPoolBuilder.when(RandomChanceLootCondition.randomChance(CONFIG.mcdwNewlootConfig.findWeaponChance));
                lootPoolBuilder.setBonusRolls(ConstantLootNumberProvider.exactly(CONFIG.mcdwNewlootConfig.bonusRollsWithLuck));
                ...
                if (COMMON_LOOT_TABLES.contains(id.toString())) {
                    ...
                    if (Arrays.stream(lootPoolBuilder.build().entries).noneMatch(lootPoolEntry ->
                            lootPoolEntry.equals(ItemEntry.lootTableItem(CrossbowsID.CROSSBOW_PRIDE_OF_THE_PIGLINS.getItem())
                                    .setWeight(CrossbowsID.CROSSBOW_PRIDE_OF_THE_PIGLINS.getItemSpawnRate()).build()))) {
    ...
    public static void addWeapon(LootPool.Builder lootPoolBuilder, Item weapon, int weight) {
        lootPoolBuilder.add(ItemEntry.lootTableItem(weapon).setWeight(weight));
    }
    ...
    public static void addItemDrop(LootPool.Builder lootPoolBuilder, Item item, int count, float probability) {
        lootPoolBuilder.setRolls(ConstantLootNumberProvider.exactly(count));
        lootPoolBuilder.when(RandomChanceLootCondition.randomChance(probability));
