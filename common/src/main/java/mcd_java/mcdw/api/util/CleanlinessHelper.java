    ...
    public static void mcdw$tooltipHelper(ItemStack stack, List<Text> tooltip, int subStringIndex) {
        ...
        if (stack.getItem() instanceof IOffhandAttack) {
            if (CompatibilityFlags.noOffhandConflicts) {
                tooltip.add(Text.translatable("tooltip_info_item.mcdw.gap").withStyle(Formatting.ITALIC));
                tooltip.add(Text.translatable("tooltip_note_item.mcdw.dualwield").withStyle(Formatting.GREEN));
            }
        }
        if (stack.getItem() instanceof McdwShortbow) {
            tooltip.add(Text.translatable("tooltip_info_item.mcdw.gap").withStyle(Formatting.ITALIC));
            tooltip.add(Text.translatable("tooltip_note_item.mcdw.shortbow").withStyle(Formatting.GREEN));
        }
        if (stack.getItem() instanceof McdwLongbow) {
            tooltip.add(Text.translatable("tooltip_info_item.mcdw.gap").withStyle(Formatting.ITALIC));
            tooltip.add(Text.translatable("tooltip_note_item.mcdw.longbow").withStyle(Formatting.GREEN));
        }
