...
public class RunicTableScreen extends AbstractContainerScreen<RunicTableScreenHandler> implements ScreenWithSlots {
    private static final ResourceLocation TEXTURE =
        new ResourceLocation(MCDEnchantments.MOD_ID, "textures/gui/runic_table.png");
    ...
    public RunicTableScreen(RunicTableScreenHandler handler, Inventory inventory, Component title) {
    ...
    protected void init() {
        ...
        slotsRenderer = EnchantmentSlotsRenderer.builder()
            ...
            .withDimPredicate(choice -> {
                ...
                return isMaxedOut || !RunicTableScreenHandler.canEnchant(minecraft.player, choice.getEnchantmentId(), level) ||
    ...
    protected void renderTooltip(GuiGraphics ctx, Choice hovered, EnchantmentSlots slots, int x, int y) {
        ...
        ResourceLocation enchantmentId = hovered.getEnchantmentId();
        List<Component> tooltipLines = new ArrayList<>();
        int level = 1;
        boolean enoughLevels = RunicTableScreenHandler.canEnchant(minecraft.player, enchantmentId, level);
        MutableComponent enchantmentName = Component.translatable(enchantment.getDescriptionId())
            .withStyle(EnchantmentUtils.formatEnchantment(enchantmentId));
        if (hovered.isChosen()) {
            enchantmentName.append(" ")
                .append(Component.translatable("enchantment.level." + hovered.getLevel()))
                .append(" ");
            if (hovered.isMaxedOut()) {
                enchantmentName.append(Component.translatable("message.mcde.max_level"));
                enoughLevels = true;
            }
            else {
                enchantmentName
                    .append("→ ")
                    .append(Component.translatable("enchantment.level." + (hovered.getLevel() + 1)));
                level = hovered.getLevel() + 1;
                enoughLevels = RunicTableScreenHandler.canEnchant(minecraft.player, enchantmentId, level);

            }
        }
        tooltipLines.add(enchantmentName);

        tooltipLines.addAll(TextWrapUtils.wrapText(width, enchantment.getDescriptionId() + ".desc", ChatFormatting.GRAY));
        if (!hovered.isMaxedOut() && !minecraft.player.isCreative()) {
            tooltipLines.addAll(TextWrapUtils.wrapText(width, Component.translatable(
        ...
        if (!enoughLevels) {
            tooltipLines.addAll(TextWrapUtils.wrapText(width, "message.mcde.not_enough_levels", ChatFormatting.DARK_RED, ChatFormatting.ITALIC));
        }

        if (!hovered.isChosen()) {
            if (EnchantmentHelper.getItemEnchantmentLevel(hovered.getEnchantment(), itemStack) > 0) {
                tooltipLines.addAll(TextWrapUtils.wrapText(width, "message.mcde.already_exists", ChatFormatting.DARK_RED, ChatFormatting.ITALIC));
                ...
                if (conflict.isPresent()) {
                    ...
                    tooltipLines.addAll(TextWrapUtils.wrapText(width, Component.translatable(
                                    ...
                                    Component.translatable(conflicting.getDescriptionId())),
                                ChatFormatting.DARK_RED, ChatFormatting.ITALIC));
                }
            }
        }
