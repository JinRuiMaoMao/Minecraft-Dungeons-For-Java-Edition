...
public class EnchantmentSlotsRenderer {
    private static final ResourceLocation missingEnchantTexture = new ResourceLocation(MCDEnchantments.MOD_ID, "textures/gui/icons/missing_no.png");
    ...
    private ResourceLocation defaultGuiTexture;
    ...
    private EnchantmentSlotsRenderer(
            ...
            ResourceLocation defaultGuiTexture,
    ...
    public void drawSlot(GuiGraphics ctx, SlotPosition slot) {
        var pos = slotPos.get(slot);
        ctx.blit(defaultGuiTexture, pos.x(), pos.y(), slotTexturePos.x(), slotTexturePos.y(), 33, 39);
    }

    public void drawChoices(GuiGraphics ctx, SlotPosition slot) {
        var pos = slotPos.get(slot).add(choicePosOffset);
        ctx.blit(defaultGuiTexture, pos.x(), pos.y(), choiceTexturePos.x(), choiceTexturePos.y(), 69, 56);
    }

    public void drawHoverOutline(GuiGraphics ctx, SlotPosition slot) {
        var pos = slotPos.get(slot);
        RenderSystem.setShaderTexture(0, defaultGuiTexture);
        ctx.blit(defaultGuiTexture, pos.x() - 1, pos.y() - 1, hoverOutlinePos.x(), hoverOutlinePos.y(), 35, 41);
    }

    public void drawIconInSlot(GuiGraphics ctx, SlotPosition slot, Choice choice) {
        var texPos = MCDEnchantments.getConfig().isEnchantmentPowerful(choice.getEnchantmentId()) ?
            powerfulOutlinePos : outlinePos;
        var pos = slotPos.get(slot);
        ctx.blit(defaultGuiTexture, pos.x(), pos.y(), texPos.x(), texPos.y(), 33, 39);
        drawIcon(ctx, pos.add(5, 6), slot, choice);
    }

    public void drawIconHoverOutline(GuiGraphics ctx, SlotPosition slot, Choice choice) {
        var drawPos = slotPos.get(slot).add(choicePosOffset).add(choiceOffsets.get(choice.getChoicePosition()));
        ctx.blit(defaultGuiTexture, drawPos.x() - 1, drawPos.y() - 1, hoverIconOutlinePos.x(), hoverIconOutlinePos.y(), 31, 33);
    }

    public void drawIconOutline(GuiGraphics ctx, SlotPosition slot, Choice choice) {
        var drawPos = slotPos.get(slot).add(choicePosOffset).add(choiceOffsets.get(choice.getChoicePosition()));
        var texPos = MCDEnchantments.getConfig().isEnchantmentPowerful(choice.getEnchantmentId()) ?
            iconPowerfulOutlinePos : iconOutlinePos;
        ctx.blit(defaultGuiTexture, drawPos.x(), drawPos.y(), texPos.x(), texPos.y(), 29, 31);
    }

    public void drawIconInChoice(GuiGraphics ctx, SlotPosition slot, Choice choice) {
    ...
    public Optional<Choice> render(GuiGraphics ctx, ItemStack itemStack, int mouseX, int mouseY) {
    ...
    private void drawIcon(GuiGraphics ctx, TexturePos drawPos, SlotPosition slot, Choice choice) {
        ResourceLocation enchantmentID = choice.getEnchantmentId();
        ResourceLocation textureID = getTextureId(enchantmentID);
        ...
        ctx.blit(Minecraft.getInstance().getResourceManager().getResource(textureID).isPresent() ?
    ...
    public static ResourceLocation getTextureId(ResourceLocation enchantmentID) {
    ...
    public static Optional<ResourceLocation> getLevelTexture(int level) {
        return Optional.of(level)
            .filter(lvl -> lvl >= 2)
            .map(lvl -> Math.min(lvl, 7))
            .map(lvl -> ResourceLocation.tryBuild(
                MCDEnchantments.MOD_ID,
    ...
    public static class Builder {
        ...
        public Builder withDefaultGuiTexture(ResourceLocation textureId) {
        ...
        public Builder withClient(Minecraft client) {
