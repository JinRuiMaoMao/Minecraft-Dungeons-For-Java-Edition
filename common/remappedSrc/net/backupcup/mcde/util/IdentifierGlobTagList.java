...
public abstract class IdentifierGlobTagList<T> extends IdentifierGlobList<T> {
    ...
    public boolean contains(ResourceLocation id) {
        return super.contains(id) ||
            ...
            .filter(tag -> tags.getOrDefault(tag.location().getNamespace(), List.of()).stream().anyMatch(glob -> glob.engine().matches(tag.location().getPath())))
    ...
    @Override
    protected ResourceLocation getId(T obj) {
        return getRegistry().getKey(obj);
    }
}
