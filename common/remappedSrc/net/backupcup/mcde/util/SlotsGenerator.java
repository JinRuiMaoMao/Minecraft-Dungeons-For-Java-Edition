...
public class SlotsGenerator {
    private ObjectArrayList<ResourceLocation> pool;
    private RandomSource random;

    private float threeChoiceChance;
    private SlotChances slotChances;

    public SlotsGenerator(ItemStack itemStack, Optional<ServerPlayer> optionalOwner, RandomSource random, float threeChoiceChance, SlotChances slotChances) {
        this.random = random;
        ...
        if (MCDEnchantments.getConfig().isCompatibilityRequired()) {
            pool.removeIf(id -> !EnchantmentUtils.isCompatible(
                EnchantmentHelper.getEnchantments(itemStack).keySet()
    ...
    public EnchantmentSlots generateEnchantments() {
        ...
        var pool = (ObjectArrayList<ResourceLocation>)Util.shuffledCopy(this.pool, random);
    ...
    private boolean generateSlot(SlotPosition pos, ObjectArrayList<ResourceLocation> pool, EnchantmentSlots.Builder builder, boolean isTwoChoiceGenerated) {
    ...
    private static SlotChances calculateAdvancementModifiers(ServerPlayer player) {
        var tracker = player.getAdvancements();
        var loader = player.server.getAdvancements();
        return MCDEnchantments.getConfig().getProgressChances().entrySet().stream()
            .filter(kvp -> Optional.ofNullable(
                loader.getAdvancement(kvp.getKey())
            ).map(a -> tracker.getOrStartProgress(a).isDone()).orElse(false))
    ...
    private void moveIncompatibleToTheEnd(ObjectArrayList<ResourceLocation> pool, int availableSlots) {
    ...
    private static List<ResourceLocation> removeIncompatible(List<ResourceLocation> pool, EnchantmentSlots.Builder builder) {
    ...
    private static float getDefaultThreeChoiceChance(ItemStack itemStack) {
        return 0.5f + itemStack.getItem().getEnchantmentValue() / 100f;
    }

    public static class Builder {
        private ItemStack itemStack;
        private Optional<ServerPlayer> optionalOwner = Optional.empty();
        private Optional<RandomSource> random = Optional.empty();
        ...
        public Builder withOwner(ServerPlayer playerEntity) {
            optionalOwner = Optional.of(playerEntity);
            return this;
        }

        public Builder withOptionalOwner(Optional<ServerPlayer> optionalPlayerEntity) {
        ...
        public SlotsGenerator build() {
            ...
            if (!(isSecondSlotChanceAbsolute && isThirdSlotChanceAbsolute) && optionalOwner.isPresent()) {
                var modifiers = SlotChances.add(
                        ...
                        calculateEnchantabilityModifier(slotChances, itemStack.getItem().getEnchantmentValue())
                        );
            ...
            return new SlotsGenerator(
                    itemStack,
                    optionalOwner,
                    random.orElseGet(() -> new SingleThreadedRandomSource(System.nanoTime())),
